<!-- Banner animado (puedes reemplazarlo por uno propio más adelante) -->
<p align="center">
  <img src="https://fgpg.com/wp-content/uploads/2022/03/Blockchain-Use-Case-Trends-1038x720-1-1038x640.jpg" alt="Banner animado" width="100%" />
</p>

<h1 align="center">👋 Hola, soy KaizoInc | Backend & Smart Contract Developer</h1>

<p align="center">
  <em>Apasionado por la tecnología, el desarrollo de software eficiente y el mundo de la blockchain 🔐</em>
</p>

---

## 🧑‍💻 Sobre mí

Soy estudiante de **Ingeniería Informática** con un gran interés en el desarrollo backend, contratos inteligentes y la tecnología blockchain. Me entusiasma construir soluciones eficientes, seguras y escalables, explorando continuamente nuevas herramientas y lenguajes para ampliar mi perspectiva como desarrollador.

🚀 Actualmente estoy enfocado en:

- Desarrollo de aplicaciones con **C++**, **Python** y **Solidity**
- Exploración de entornos blockchain como **Foundry** y herramientas de **Chainlink**
- Optimización de código y estructuras algorítmicas

---

## 🛠️ Tecnologías y herramientas

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="40" alt="C++"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="40" alt="Python"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/solidity/solidity-original.svg" width="40" alt="Solidity"/>
  <img src="https://raw.githubusercontent.com/foundry-rs/foundry/master/media/foundry-logo.svg" width="40" alt="Foundry" title="Foundry"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="40" alt="Git"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="40" alt="Linux"/>
</p>

---

## 📈 Estadísticas de GitHub

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=KaizoIncc&show_icons=true&theme=radical" alt="GitHub Stats" height="160" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=KaizoIncc&layout=compact&theme=radical" alt="Top Languages" height="160" />
</p>

---

## 🌟 Proyectos destacados

🔗 **[Foundry Lottery](https://github.com/KaizoIncc/Foundry-Lottery)**  
*Lotería descentralizada usando Foundry, Chainlink VRF v2 y Chainlink Automation.*

🔗 **[INEP_PROJECT](https://github.com/KaizoIncc/INEP_PROJECT)**  
*Aplicación de terminal para gestionar visualización de series y películas como proyecto académico.*

🔗 **[Arduino – CAN BUS Shield](https://github.com/KaizoIncc/Arduino---CAN-BUS-Shield)**  
*Comunicación entre dispositivos vía bus CAN usando Arduino y el CAN BUS Shield de Seeed Studio.*

---

## 📬 Contacto

Puedes encontrarme y conectar a través de los siguientes medios:

- 📧 Email: [kaizoinc22@gmail.com](mailto:kaizoinc22@gmail.com)
- 💼 LinkedIn: [Aymane El Hisati El Aissati](https://www.linkedin.com/in/aymane-el-hisati-el-aissati-56105935a/)
- 💻 GitHub: [KaizoIncc](https://github.com/KaizoIncc)
- 💬 Discord: `bbq_kaizoinc`

---

## 🧠 Más allá del código

Fuera del teclado, me encanta:

♟️ Jugar ajedrez y damas  
🕹️ Juegos de estrategia como **LoL**, shooters como **CS:GO**, **Valorant** y otros títulos variados  
🌲 Explorar la naturaleza y caminar al aire libre  

---

## 💡 Filosofía y motivación

> "Cada línea de código es una oportunidad para construir el futuro, pero la verdadera magia ocurre cuando combinamos lógica con pasión." 🔥

---

⭐ **Si te interesa mi trabajo, no dudes en seguirme y explorar mis repositorios. Estoy abierto a colaboraciones y nuevos desafíos.**

---



<!--
**KaizoIncc/KaizoIncc** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
